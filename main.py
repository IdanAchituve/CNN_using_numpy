...
nn_params = {}
nn_params["model"] = "CNN"
nn_params["optimizer"] = "SGD"  # optimizer
nn_params["augmentation"] = False
nn_params["lr"] = 0.01
nn_params["lr_decay_epoch"] = 100000  # learning rate decay epoch
nn_params["momentum"] = 0.0
nn_params["momentum_change_epoch"] = 10000  # momentum increase epoch
...
nn_params["operation"] = ["conv", "pol"]  # type of operation on layer
nn_params["conv_num_layers"] = [3, 16]  # number of feature maps to create in each convolution operation. Must start with 3
nn_params["stride"] = [1]  # conv stride
nn_params["filter_size"] = [3]  # filter size. only on convolution layers
nn_params["conv_activation"] = ["relu"]
nn_params["padding"] = [1]  # padding for each convolution layer
nn_params["pooling_stride"] = [2]  # pooling window size
nn_params["layers"] = [4096, 100, 10]  # FC dims - 1st dim is the flattening of the final conv/pol layers
